...
Given a set of 2D coordinates, determine if a proposed distance $d_{proposed}$ is the shortest possible route subject to decimal precision $p_{decimal}$. A valid route reaches every coordinate once and returns to the starting coordinate.
...
\newpage
\section{Factorial Storage Problem}
System algorithm that is not bound by polynomial time.
%\section{Contrived Algorithm Proving the Existence of $\mathcal{N}$}
\subsection{English Definition of Factorial Storage Problem}
Given a finite set of inputs $\{x_1,x_2,...,x_N\}$, what is every possible permutation of inputs $x_i$?
\subsection{Formal Definition}
Given inputs $x_i$
\begin{center}
$
...
\\ \vspace{6mm}
\end{center}
Define the Factorial Storage Problem Q
\begin{center}
$
Q := f[X_i] = Y_o = \{Y_1,Y_2,...,Y_{i!}\} \hspace{2mm} \forall X_i:
$
\\ \vspace{6mm}
$
\forall j,k,...,m \leq n \hspace{2mm} \land \hspace{2mm}  j \neq k \neq ... \neq m
$
\\ \vspace{2mm}
$
\{x_j,x_k,...,x_m\} \in Y_o 
$
\end{center}
\subsection{Prove the Factorial Storage Problem is a Problem}
Suppose the Factorial Storage Problem is not a problem\\ \\
Let
\begin{center}
$
Q := f[X_i] \not \subseteq \Omega \hspace{2mm}
$
\end{center}
By definition of Factorial Storage Problem
\begin{center}
$
f[X_i] = \{Y_1,Y_2,...,Y_{i!}\} \hspace{2mm} \forall X_i
$
\end{center}
Then
\begin{center}
...
$
\end{center}
Therefore the Factorial Storage Problem is a Problem by contradiction of $\Omega$
\begin{center}
$
\therefore Q := f[X_i] \subseteq \Omega \hspace{4mm} \forall X_i \Longrightarrow Q \in \mathbb{Q}
$
\end{center}
\subsection{Prove the Factorial Storage Problem is a System Problem}
Suppose the Factorial Storage Problem is not in the set of System Problems\\ \\
Let
\begin{center}
$
f[X_i] \neq \{y_1,y_2,...,y_m\} \hspace{4mm} (1)
$
\end{center}
\subsection{Define a solution $s^+$}
\subsection{Define Subfunctions for $s^+$}
\subsection{Prove each subfunction satisfies the subfunction condition of solutions}
\subsection{Prove the overall solution satisfies the subfunction condition of solutions}
\subsection{Express O[n] in term of subfunction complexities}

\subsection{Prove subfunctions are non-polynomial}
By contradiction, cannot be bounded by a polynomial function

\subsection{Prove the solution is non-polynomial}
By the theorem of non-polynomial subfunctions (go back and check theorem of non-polynomial subfunctions)

\subsection{Prove there does not exist solution $s^+$ with polynomial bound}
By contradiction
